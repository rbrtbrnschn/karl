import { common, parseString, stringToCountMap, stripHTMLEntities } from './common';
...
describe('translateHTMLEntities', () => {
  it("should translate", ()=> {
    expect(stripHTMLEntities(`Hi.&nbsp;We are Pete &amp; David.`)).toEqual("Hi. We are Pete & David.")
  })
  it("should not translate",()=> {
    expect(stripHTMLEntities(`3,1415 &asymp; 3,14`)).toEqual(`3,1415 &asymp; 3,14`);
  });
})
...
describe('stringToCountMap',() => {
  ...
  it("should be equal to parsed countmap",()=>{
    expect(stringToCountMap(parseString(sentenceOne))).toEqual(parsedCountMapOne);
  })
  it("[CUSTOM SEPERATOR] - should be equal",()=>{
    expect(stringToCountMap(sentenceOne,".")).toEqual({"Hello World":1, " Hello Pete":1})
  })
})
...
describe('parseString',()=>{
  ...
  it("[CUSTOM REGEX] - should leave quotes untouched",()=>{
    expect(parseString(sentenceOne,/(<([^>]+)>)/gi,/[.,;:!?()[\]{}\\$/*&\-\–\s]/g)).toEqual('html 5 "boilerplate"')
  })
})
